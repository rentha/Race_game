...
YELLOW_CAR = pygame.image.load("imgs/car-yellow.png")
...
class GameInfo:
    ...
    def get_best_time(self):   
        self.prev_time = round(time.time() - self.level_start_time)
        if best_time < self.prev_time:
            return best_time == self.prev_time
        print(self.get_best_time)
    ...
    """def get_best_time(self):
        level_time = round(time.time() - self.level_start_time)
        best_time = self.level_start_time
        if level_time < best_time:
            level_time == best_time
        else:
            return self.get_level_time"""
...
def draw(win, images, player_car, game_info):
    ...
    best_time_text = MAIN_FONT.render (f"Best time: {game_info.get_best_time}s", 1, (255, 255, 255))
